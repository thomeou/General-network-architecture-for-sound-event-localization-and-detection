...
def interpolate_tensor(tensor, ratio: float = 1.0):
    ...
    :param tensor: (batch_size, n_timesteps, n_classes) or (n_timesteps, n_classes). Torch tensor.
    :param: ratio. If ratio > 1: upsample, ratio < 1: downsample # ratio = output rate/input rate
    :return: new_tensor (batch_size, n_timestepss*ratio, n_classes) or (n_timestepss*ratio, n_classes)
    """
    ratio = float(ratio)
    n_dims = tensor.ndim

    if n_dims == 2:
        n_input_frames, n_classes = tensor.shape[0], tensor.shape[1]
        ...
        batch_size, n_input_frames, n_classes = tensor.shape[0], tensor.shape[1], tensor.shape[2]
        ...
        raise NotImplementedError('Interpolate function does not work for tensor dimension above 3')
...
class AttBlock(nn.Module):
    ...
    def forward(self, x):
        # x: (n_samples, n_channels/n_features, n_time_steps)
        # norm_att = torch.softmax(torch.clamp(self.att(x), -10, 10), dim=-1)
        norm_att = torch.softmax(torch.tanh(self.att(x)), dim=-1)  # (batch_size, n_classes, n_time_steps) softmax on n_time_steps dimension
        cla = self.nonlinear_transform(self.cla(x))  # (batch_size, n_classes, n_time_steps)
        x = torch.sum(norm_att * cla, dim=2)  # sum over time dimension (batch_size, n_classes)
...
class PositionalEncoding(nn.Module):
    def __init__(self, pos_len, d_model=512, pe_type='t', dropout=0.1):
