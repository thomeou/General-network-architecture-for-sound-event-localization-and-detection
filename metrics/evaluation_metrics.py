...
Some functions has been modified from the original functions.
"""
#
...
azimuths = range(-180, 180, 1)
elevations = range(-45, 46, 1)
##########################################################################################
    ...
    def update_doa_scores(self, pred_doa_thresholded, gt_doa):
        ...
        for frame in range(pred_doa_thresholded.shape[0]):
            ...
            if nb_gt_peaks and nb_pred_peaks:
                ...
                pred_list_rad = np.array(get_matrix_index(pred_ind, azimuths, elevations)) * np.pi / 180

                gt_ind = np.where(gt_doa[frame] == 1)[1]
                # gt_list_rad = np.array(self._data_gen .get_matrix_index(gt_ind)) * np.pi / 180
                gt_list_rad = np.array(get_matrix_index(gt_ind, azimuths, elevations)) * np.pi / 180
...
def reshape_3Dto2D(A):
    """
    :param A: (batch_size, n_timesteps, n_classes)
    :return: (batch_size * n_timesteps, n_classes)
    """
...
def er_overall_framewise(O, T):
    ...
    Nref = T.sum()
    ER = (S+D+I) / (Nref + 0.0)
    return ER, S/Nref, D/Nref, I/Nref
...
def compute_sed_scores(pred, gt, nb_frames_1s):
    ...
    scores = [ero[0], f1o[0], ero[1], ero[2], ero[3], f1o[1], f1o[2]]
    return scores
...
def compute_doa_scores_regr(pred_doa_rad, gt_doa_rad, pred_sed, gt_sed):
    ...
    frame_recall = good_frame_cnt / float(gt_sed.shape[0])
    er_metric = [doa_loss_pred, frame_recall, doa_loss_pred_cnt, good_frame_cnt/float(gt_sed.shape[0]),
                 more_est_cnt/float(gt_sed.shape[0]), less_est_cnt/float(gt_sed.shape[0])]
    return er_metric
...
###################################################################
def get_matrix_index(ind, azimuths, elevations):
    n_eles = len(elevations)
    azi_reln = int(abs(azimuths[1] - azimuths[0]))
    ele_reln = int(abs(elevations[1] - elevations[0]))
    
    azi, ele = ind // n_eles, ind % n_eles
    azi = (azi * azi_reln + azimuths[0])
    ele = (ele * ele_reln + elevations[0])
    return azi, ele
